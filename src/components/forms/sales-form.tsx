...
import { addSaleAction } from "@/lib/actions";
...
function SubmitButton() {
  ...
  return (
    <Button type="submit" disabled={pending} className="w-full sm:w-auto">
      {pending ? <Loader2 className="animate-spin" /> : "Record Sale"}
    </Button>
  );
}

export function SalesForm({ stockItems }: SalesFormProps) {
  ...
  const [state, formAction] = useActionState(addSaleAction, { success: false, message: "" });
  ...
  const handleBookChange = (bookId: string) => {
    ...
    form.setValue("bookId", bookId, { shouldValidate: true });
    const stock = stockItems.find((item) => item.id === bookId)?.closingQty;
    ...
    return (
      <div className="flex flex-col items-center justify-center text-center p-10 bg-secondary rounded-lg border border-dashed">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          You must first add a book to the master list before you can record a sale.
        </p>
  ...
  return (
    <form action={formAction} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {selectedBookId && (
          <div className="p-3 bg-secondary rounded-md">
            <p className="text-sm font-medium text-secondary-foreground">Available Stock</p>
            <p className="text-2xl font-bold font-headline">{availableStock}</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div>
          <Label htmlFor="time">Time of Sale</Label>
